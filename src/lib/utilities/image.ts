...
export const fetchEmbedding = async (text: string): Promise<number[]> => {
	try {
		...
		throw new Error(`Error fetching embedding for text: ${text}`, { cause: error });
	}
};

export const getImageDescription = async (base64Img: string, prompt: string): Promise<string> => {
	const requestBody = {
		model: 'gpt-4o',
		messages: [
			{
...
export async function getBase64Image(imgPath: string): Promise<string> {
	...
	try {
		...
		try {
			const jpegBuffer = await sharp(Buffer.from(imgBuffer)).jpeg({ quality: 80 }).toBuffer();
...
export async function processImage(imgPath: string, sampleTags: string[]): Promise<imageMeta> {
	try {
...
export async function processImages(
	...
	for (const filePath of filePaths) {
		try {
			...
			throw new Error(`Error processing image ${filePath}: ${(error as Error).message}`);
		}
	}
