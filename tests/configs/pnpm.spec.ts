...
if (isJsdom) {
  describe.skip('PNPM Setup', () => {
    it('Tests are skipped in jsdom environment.', () => {
      console.warn('PNPM Setup tests are skipped in jsdom environment.');
    });
  });
} else {
  // Use `require()` for Node.js modules when in Node environment
  ...
  describe('PNPM Setup', () => {
    ...
    it('should execute pnpm build without critical errors', () => {
      const options = { stdio: 'pipe', timeout: 300000 }; // 5 minutes timeout
      try {
        ...
        if (error instanceof Error) {
          ...
          if (
            ...
            !(
              ...
              error.stderr
                .toString()
                .includes('Command failed with exit code 1')
            )
          ) {
            throw error;
          }
    ...
    it('should warn if dependencies are not up to date', () => {
      const options = { stdio: 'pipe', timeout: 60000 }; // 1 minute timeout
      try {
